...
class NIRAdapter:
    """Thin, fail-closed adapter around the official NIR Python package.

    NIR stores graph structure and parameters; execution details that are not
    fixed by the graph live in a hashed ExecutionSemantics sidecar.
    """
    ...
    def _module() -> Any:
        try:
            ...
            raise RuntimeError(
                "NIR support is optional; install transportcert[nir] and pin the version"
            ) from error
