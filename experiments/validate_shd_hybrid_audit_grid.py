...
def main() -> None:
    ...
    if args.grid_resolution < 2 or args.batch_size < 1:
    ...
    for seed in config["seeds"]:
        ...
        for audit_row, frame, reference_prediction in zip(
            seed_rows, frames, reference_predictions, strict=True
        ):
            ...
            mismatch = execution.predictions != reference_prediction
            output_rows.append(
                {
                    "seed": int(seed),
                    ...
                    "reference_prediction": int(reference_prediction),
                    "grid_identity": not bool(np.any(mismatch)),
                    "counterexample_point_count": int(np.count_nonzero(mismatch)),
