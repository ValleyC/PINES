...
Each input uses a fresh allocation. Ten independent model populations receive
...
def run_input(sim, models, event, sample_id, index, output, args, mapping):
    from spinn_utilities.config_holder import set_config, get_config_bool
    ...
    try:
        ...
        for (seed, variant, model), hidden in zip(models, hidden_populations):
            segment = hidden.get_data("spikes").segments[-1]
        ...
        return dict(status="physical_capture_completed", sample_id=sample_id, input_index=index,
...
def run(args):
    ...
    config = dict(dataset=metadata, inputs_file=args.inputs.name,
                  ...
                  sampling="one unique input per fresh allocation, shared across conditions",
                  independence="Independent inputs and allocation-level hardware noise are assumed. Conditions need not be independent.",
    ...
    for index in range(args.start, stop):
        ...
        folder.mkdir()
        result = run_input(sim, models, events[index], str(ids[index]), index, folder, args, mapping)
    ...
    (args.output / "completed.json").write_text(json.dumps(dict(samples=args.count,
        conditions=len(models), status="physical_matrix_capture_completed"), indent=2) + "\n")
